...
async def refresh(body: RefreshIn, db: AsyncSession = Depends(get_db)) -> JSONResponse:
    ...
    try:
        result = await service.refresh_tokens(db=db, refresh_token=body.refresh_token)
    except AppError as exc:
        raise exc.as_http_exception()
    return JSONResponse(status_code=status.HTTP_200_OK, content=result)
...
async def logout(body: RefreshIn, db: AsyncSession = Depends(get_db)) -> JSONResponse:
    """Revoke the provided refresh token, invalidating the session."""
    result = await service.logout_user(db=db, refresh_token=body.refresh_token)
    return JSONResponse(status_code=status.HTTP_200_OK, content=result)
...
async def verify_otp(body: VerifyOtpIn, db: AsyncSession = Depends(get_db)) -> JSONResponse:
    ...
    try:
        result = await service.verify_otp(
            db=db, email=body.email, otp=body.otp, purpose=body.purpose.value
        )
    except AppError as exc:
        raise exc.as_http_exception()
    return JSONResponse(status_code=status.HTTP_200_OK, content=result)
