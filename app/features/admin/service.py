...
async def _get_role_or_raise(db: AsyncSession, role_id: int) -> Role:
...
async def _get_permission_or_raise(db: AsyncSession, permission_id: int) -> Permission:
    perm = await repo.get_permission_by_id(db=db, permission_id=permission_id)
    if not perm:
...
async def update_role(db: AsyncSession, role_id: int, name: str | None, description: str | None) -> Role:
    ...
    if role.name in RoleNames.ALL:
        raise BadRequestError(ErrorCodes.FORBIDDEN, ResponseMessages.CANNOT_MODIFY_DEFAULT_ROLE)
...
async def assign_permission_to_role(db: AsyncSession, role_id: int, permission_id: int) -> None:
    ...
    await _get_role_or_raise(db, role_id)
    await _get_permission_or_raise(db, permission_id)
...
async def get_user_roles(db: AsyncSession, user_id: int) -> User:
    ...
    return await _get_user_or_raise(db, user_id)
...
async def assign_role_to_user(db: AsyncSession, user_id: int, role_id: int) -> None:
    ...
    await _get_user_or_raise(db, user_id)
    await _get_role_or_raise(db, role_id)
